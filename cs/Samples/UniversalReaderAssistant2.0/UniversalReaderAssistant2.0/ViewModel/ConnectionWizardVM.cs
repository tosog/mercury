...
namespace ThingMagic.URA2.ViewModel
{
    ...
    public class ConnectionWizardVM : ViewModelBase
    {
        ...
        private Visibility _antennaLabelHFVisibility;

        public Visibility AntennaLabelHFVisibility
        {
            get { return _antennaLabelHFVisibility; }
            set { _antennaLabelHFVisibility = value; RaisePropertyChanged("AntennaLabelHFVisibility"); }
        }


        private string _antennaContent1;

        public string AntennaContent1
        {
            get { return _antennaContent1; }
            set { _antennaContent1 = value; RaisePropertyChanged("AntennaContent1"); }
        }
        ...
        private Visibility _antennaLabelLFVisibility;

        public Visibility AntennaLabelLFVisibility
        {
            get { return _antennaLabelLFVisibility; }
            set { _antennaLabelLFVisibility = value; RaisePropertyChanged("AntennaLabelLFVisibility"); }
        }


        private string _antennaContent2;

        public string AntennaContent2
        {
            get { return _antennaContent2; }
            set { _antennaContent2 = value; RaisePropertyChanged("AntennaContent2"); }
        }
                    ...
                    else if (UserControlIndex == 1)
                    {
                        if (ValidateConnectReaderPage())
                        {
                            ...
                            foreach (bool temp in AntennaCheckedBool)
                            {
                                if (temp)
                                {
                                    string ant = ((tempint.ToString().Equals("0")) ? "HF" : "LF");
                                    DetectedSelectedAntenna = DetectedSelectedAntenna + ((DetectedReaderModel.Equals("M3e")) ? ant : (tempint + 1).ToString()) + ",";
                                }
                                tempint++;
                            }
        ...
        private void HFFAntennaCheck(string protocolType, bool value)
        {
            if (protocolType.Equals("HF"))
            {
                if (value)
                {
                    AntennaIsChecked1 = true;
                    AntennaLabelHFVisibility = Visibility.Visible;
                }
                else
                {
                    if ((ISO14443AIsChecked | ISO14443BIsChecked | ISO15693IsChecked | ISO18092IsChecked | FELICAIsChecked | ISO180003Mode3IsChecked) == false)
                    {
                        AntennaIsChecked1 = false;
                        AntennaLabelHFVisibility = Visibility.Collapsed;
                    }
                }
            }
            else
            {
                if (value)
                {
                    AntennaIsChecked2 = true;
                    AntennaLabelLFVisibility = Visibility.Visible;
                }
                else
                {
                    if ((LF125KHZIsChecked | LF134KHZIsChecked) == false)
                    {
                        AntennaIsChecked2 = false;
                        AntennaLabelLFVisibility = Visibility.Collapsed;
                    }
                }
            }
        }
        ...
        public void ConfigureAntennaBoxes()
        {
            ...
            else
            {
                ...
                for (int i = 0; i < ANTENNACOUNT; i++)
                {
                    ...
                    {
                        case 0:
                            if (!(model1.Equals("M3e")))
                            {
                                AntennaLabelLFVisibility = AntennaLabelHFVisibility = Visibility.Collapsed;
                            ...
                            {
                                AntennaVisibility1 = Visibility.Collapsed;//(existingAntennas.Contains(i + 1)) ? Visibility.Visible : Visibility.Collapsed;
                            }
                            break;
                        case 1:
                            if (!(model1.Equals("M3e")))
                            ...
                            {
                                AntennaVisibility2 = Visibility.Collapsed;//(existingAntennas.Contains(i + 1)) ? Visibility.Visible : Visibility.Collapsed;
                            }
                            break;
                        case 2:
